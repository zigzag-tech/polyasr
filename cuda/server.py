...
async def startup_event():
    ...
    log.info("Runtime preflight passed: %s", versions)
    # Validate the inexpensive CPU-side dependencies before allocating several
...
def align_local_path(path: Path, language: Optional[str], max_chunk_seconds: int) -> dict:
    """Align one local audio file (chunked ≤ max_chunk_seconds) and return
    {text, language, segments} (NO 'model' key — callers add it). Runs on a
    worker thread while holding _transcribe_lock (load + per-chunk transcribe
    serialized). Shared by /v1/align and /v1/align/manifest."""
    audio_path = path
...
async def align(
    ...
    `model` is accepted for API symmetry but the server's configured
    POLYASR_MODEL/POLYASR_ALIGNER_MODEL are authoritative."""
    suffix = Path(file.filename).suffix if file.filename else ".wav"
    tmp = tempfile.NamedTemporaryFile(delete=False, suffix=suffix)
    try:
        content = await file.read()
